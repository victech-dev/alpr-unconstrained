...
class ShapeDisplay(Shape):
    ...
    def draw(self,
        ...
        if ss > 1:
            for i in range(ss):
                drawLineFunc(self.pts[:, i],
                             self.pts[:, (i + 1) % ss],
                             color=color)
                drawCircleFunc(self.pts[:, 0],
                               color=(255 - color[0], 0, 255 - color[2]))
                drawCircleFunc(self.pts[:, -1],
                               color=(255 - color[0], 255 - color[1],
                                      255 - color[2]))


def rotation_transform(wh,
    ...
    a = rads[0]
    Rx = np.matrix([[1, 0, 0], [0, cos(a), sin(a)], [0, -sin(a),
                                                     cos(a)]])
    a = rads[1]
    Ry = np.matrix([[cos(a), 0, -sin(a)], [0, 1, 0], [sin(a), 0,
                                                      cos(a)]])
    a = rads[2]
...
class Display():
    ...
    def updatePerspectiveMatrix(self):
        ...
        R = np.matmul(
            R,
            np.matrix([[zf, 0, -self.dx], [0, zf, -self.dy], [0, 0, 1]],
                      dtype=float))
    ...
    def _setPerspective(self, update=True):
        ...
        self.IdisplayCopy = cv2.warpPerspective(self.Iorig,
                                                self.R,
...
def displayAllShapes(disp, shapes, selected, typing_mode):
    for i, shape in enumerate(shapes):
        ...
        txtcolor = (0, 0,
                    255) if (i == selected and typing_mode) else (255, 255,
                                                                  255)
        shape.draw(disp.drawLine,
                   disp.drawCircle,
                   disp.drawText,
                   color=color,
                   txtcolor=txtcolor)
